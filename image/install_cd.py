...
class InstallCD:
  ...
  def _get_install_cd(self):
    if not os.path.isfile(self._install_cd):
      ...
      infile = urllib2.urlopen(self._config.install_cd_url)
  ...
  def _extract_install_cd(self):
    ...
    proc = subprocess.Popen(['gksudo', '--', 'mount', '-o', 'loop,ro', self._install_cd, self._mnt_dir])
    proc.communicate()

    shutil.copytree(self._mnt_dir, self._iso_dir, symlinks=True)

    proc = subprocess.Popen(['gksudo', '--', 'umount', self._mnt_dir])
    proc.communicate()
